...
class ShapeChecker:
    """
    Checks shape of tensors during runtime.
    ShapeChecker keeps track of named dims internally. A literal once assigned to a dim is immutable.

    Usage:
    sc.check(x, "B C H W")

    Give known sizes:
    sc.check(x, "B Cin H W", Cin=3)

    Do math:
    sc.check(x, "B C H*W")
    sc.check(x, "B Cin+Cout H W")
    """
    ...
    def check(self, tensor: Tensor, spec: Spec, **known_dims):
        ...
        for i, dim in sorted(enumerate(spec), key=lambda x: self._is_composite(x[1])):
            if isinstance(dim, int) or (isinstance(dim, str) and dim.isnumeric()):
                ...
                raise TypeError(
                    f"Found type {type(dim)}, dims must be str or int")
